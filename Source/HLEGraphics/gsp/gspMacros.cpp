...
void DLParser_GBI2_Mtx( MicroCodeCommand command )
{
        ...
        u32 mtx_command = (command.cmd0)&0xFF;
        u32 length      = (command.cmd0 >> 8)&0xFFFF;

        use(length);

        DL_PF("    Command: %s %s %s Length %d Address 0x%08x",
                (mtx_command & G_GBI2_MTX_PROJECTION) ? "Projection" : "ModelView",
                (mtx_command & G_GBI2_MTX_LOAD) ? "Load" : "Mul",
                (mtx_command & G_GBI2_MTX_NOPUSH) ? "NoPush" : "Push",
                length, address);
        ...
        if (mtx_command & G_GBI2_MTX_PROJECTION)
        {
                // So far only Extreme-G seems to Push/Pop projection matrices  
                PSPRenderer::Get()->SetProjection(mat, push, load_command);
        }
...
{
        u8 mtx_command = (u8)(command.cmd1 & 0xFF);

        DL_PF("    Command: 0x%02x (%s)",
                mtx_command,  (mtx_command & G_GBI1_MTX_PROJECTION) ? "Projection" : "ModelView");
...
{ 
		...
		u32 code_base = RDPSegAddr(command.cmd1);
        u32 code_size = 0x1000; 
        u32 data_base = RDPSegAddr(*(u32 *)(g_pu8RamBase + pc-12));
        u32 data_size = u32((RDPSegAddr(command.cmd1)) & 0xffff) + 1; 
